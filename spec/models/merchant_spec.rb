...
RSpec.describe Merchant, type: :model do
  ...
  describe 'instance methods' do
    before :each do 
      @merchant_1 = Merchant.create!(name: 'Schroeder-Jerde', status: "enabled")
      @merchant_2 = Merchant.create!(name: 'Rempel and Jones', status: 'disabled')
      @merchant_3 = Merchant.create!(name: 'Willms and Sons', status: 'enabled')
      ...
      @invoice_item_19 = InvoiceItem.create!(invoice_id: @invoice_19.id, item_id: @item_2.id, quantity: 9, unit_price: 76941, status: 'packaged')
      @invoice_item_20 = InvoiceItem.create!(invoice_id: @invoice_20.id, item_id: @item_2.id, quantity: 9, unit_price: 76941, status: 'packaged')
      @invoice_item_21 = InvoiceItem.create!(invoice_id: @invoice_21.id, item_id: @item_2.id, quantity: 9, unit_price: 76941, status: 'packaged')
      @invoice_item_22 = InvoiceItem.create!(invoice_id: @invoice_22.id, item_id: @item_3.id, quantity: 9, unit_price: 76941, status: 'pending')
      @invoice_item_23 = InvoiceItem.create!(invoice_id: @invoice_23.id, item_id: @item_4.id, quantity: 9, unit_price: 76941, status: 'shipped')
    end

    it '#find_by_status' do 
      enabled_merchants = Merchant.find_by_status('enabled')
      expect(enabled_merchants.to_a).to eq([@merchant_3, @merchant_1])

      disabled_merchants = Merchant.find_by_status('disabled')
      expect(disabled_merchants.to_a).to eq([@merchant_2])
    end
  end
